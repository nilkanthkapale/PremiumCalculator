...
namespace PC.Business.MudBlazorCustomization.MudBlazorCustomization
{
	...
	/// </summary>
	public partial class MudSelectItemCustom<T> : MudBaseSelectItem, IDisposable
	{
		...
		internal IMudShadowSelect IMudShadowSelect
		{
			...
			{
				_shadowParent = value;
				((MudSelectCustom<T>)_shadowParent)?.RegisterShadowItem(this);
			}
		}
		...
		internal MudSelectCustom<T> MudSelect => (MudSelectCustom<T>)IMudSelect;
		...
		/// <summary>
		/// String value defined for user input search
		/// </summary>
		[Parameter]
		...
		public void Dispose()
		{
			try
			{
				...
				((MudSelectCustom<T>)_shadowParent)?.UnregisterShadowItem(this);
			}
			catch (Exception) { }
		}
	}
