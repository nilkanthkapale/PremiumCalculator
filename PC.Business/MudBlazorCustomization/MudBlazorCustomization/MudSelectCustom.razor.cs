...
{
	/// <summary>
	/// Copied from Github https://github.com/MudBlazor/MudBlazor/tree/dev/src/MudBlazor/Components/Select to solve "search" issue
	/// Once PR: https://github.com/MudBlazor/MudBlazor/pull/5988 got merged into MudBlazor this folder and component usage can be removed and update to latest Nuget package..
	/// </summary>
	/// <typeparam name="T"></typeparam>
	public partial class MudSelectCustom<T> : MudBaseInput<T>, IMudSelect, IMudShadowSelect
	{
		...
		private async Task SelectAdjacentItem(int direction)
		{
			...
			if (direction < 0 && index < 0)
				index = 0;
			MudSelectItemCustom<T> item = null;
		...
		private ValueTask ScrollToItemAsync(MudSelectItemCustom<T> item)
		...
		private async Task SelectFirstItem(string input = null)
		{
			...
			_logger.LogInformation($"SelectFirstItem - Search: {input}");
			...
			if (!string.IsNullOrWhiteSpace(input))
			{
				...
				if (currentItem != null && currentItem.SearchValue?.ToLowerInvariant().Contains(input) == true)
				{
				...
				items = items.Where(x => x.SearchValue?.ToLowerInvariant().Contains(input) == true);
			}
			var item = items.FirstOrDefault();
			_logger.LogInformation($"SelectFirstItem - items.Count: {items.Count()}, item: {(item == null ? "NULL" : item?.SearchValue?.ToString())}");
		...
		/// </summary>
		public IReadOnlyList<MudSelectItemCustom<T>> Items => _items;

		protected internal List<MudSelectItemCustom<T>> _items = new List<MudSelectItemCustom<T>>();
		protected Dictionary<T, MudSelectItemCustom<T>> _valueLookup = new Dictionary<T, MudSelectItemCustom<T>>();
		protected Dictionary<T, MudSelectItemCustom<T>> _shadowLookup = new Dictionary<T, MudSelectItemCustom<T>>();

		// note: this must be object to satisfy MudList
		private object _activeItemId = null;

		internal bool Add(MudSelectItemCustom<T> item)
		{
			if (item == null)
				return false;
			bool? result = null;
		...
		internal void Remove(MudSelectItemCustom<T> item)
		{
			_items.Remove(item);
			if (item.Value != null)
		...
		[Inject] private ILogger<MudSelect<T>> _logger { get; set; }
		...
		private async void HilightItem(MudSelectItemCustom<T> item)
		{
		...
		internal async void HandleKeyDown(KeyboardEventArgs obj)
		{
			...
			if (_isOpen && key.Length == 1 && key != " " && !(obj.CtrlKey || obj.ShiftKey || obj.AltKey || obj.MetaKey))
			{
				...
				_searchChars += key;
				_logger.LogInformation($"HandleKeyDown - Key: {key}, Search: {_searchChars}");
		...
		public void RegisterShadowItem(MudSelectItemCustom<T> item)
		{
		...
		public void UnregisterShadowItem(MudSelectItemCustom<T> item)
		{
			if (item == null || item.Value == null)
				return;
